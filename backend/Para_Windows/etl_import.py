...
DB_PORT = "5433"
DB_NAME = "cnpj_dados"
# Timeout aumentado para garantir estabilidade em conexões lentas
DATABASE_URL = f"postgresql+pg8000://{DB_USER}:{DB_PASS}@{DB_HOST}:{DB_PORT}/{DB_NAME}"
PASTA_DADOS = "./dados"
TAMANHO_CHUNK = 20000  # Tamanho seguro para 16GB RAM
...
def limpar_dados(df):
    """Limpa caracteres nulos e espaços extras que quebram o CSV"""
    df = df.fillna("")
...
def inserir_via_copy(conn, df, tabela):
    """Método ultra-rápido usando COPY do Postgres"""
    output = io.StringIO()
...
def processar_csv(caminho_csv, tabela, colunas):
    ...
    # Configuração de sessão para performance
    try:
...
def processar_zip(caminho_zip, tabela, colunas):
    ...
    try:
        with zipfile.ZipFile(caminho_zip, 'r') as z:
            ...
            os.remove(caminho_csv) # Limpa o CSV extraído para economizar espaço
    except Exception as e:
...
def main():
    ...
    # Pega apenas arquivos ZIP
    arquivos = sorted([f for f in os.listdir(PASTA_DADOS) if f.endswith(".zip")])
    
    print(f"--- Iniciando Importação ({len(arquivos)} arquivos encontrados) ---")
    print("OBS: O script processa e apaga os CSVs extraídos automaticamente.")
    ...
    for arquivo in arquivos:
        ...
        else: print(f"[Ignorado] {arquivo} (Não reconhecido)")

    print("\n[FIM] Importação concluída!")
