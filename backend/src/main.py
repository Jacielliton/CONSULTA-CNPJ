...
# Função para ler o usuário a partir do Token
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    with engine.connect() as conn:
        user = conn.execute(text("SELECT id, nome, email, contato FROM users WHERE email = :e"), {"e": email}).mappings().fetchone()
...
def register(user: UserCreate):
    with engine.connect() as conn:
        ...
        conn.execute(text("""
            INSERT INTO users (nome, email, senha_hash, contato) 
            VALUES (:n, :e, :s, :c)
...
def login(user: UserLogin):
    ...
    access_token = create_access_token(data={"sub": db_user['email'], "name": db_user['nome']})
    return {"access_token": access_token, "token_type": "bearer", "user_name": db_user['nome']}
...
@app.get("/buscar")
def buscar_empresa(q: Optional[str]=None, page: int=1, limit: int=10, uf: Optional[str]=None, municipio: Optional[str]=None, situacao: Optional[str]=None, data_inicio: Optional[str]=None, data_fim: Optional[str]=None):
    offset = (page - 1) * limit
...
@app.get("/exportar")
def exportar_dados(q: Optional[str]=None, uf: Optional[str]=None, municipio: Optional[str]=None, situacao: Optional[str]=None, data_inicio: Optional[str]=None, data_fim: Optional[str]=None):
    cond = ["1=1"]
    p = {}
    ...
    def iterar():
        ...
        with engine.connect() as conn:
            for row in conn.execution_options(yield_per=2000).execute(text(sql), p):
                ...
                l = [
                    fmt_cnpj(r['cnpj_basico'], r['cnpj_ordem'], r['cnpj_dv']),
...
def atualizar(bg: BackgroundTasks):
    bg.add_task(executar_script, "etl_import.py")
    return {"status": "ok"}
